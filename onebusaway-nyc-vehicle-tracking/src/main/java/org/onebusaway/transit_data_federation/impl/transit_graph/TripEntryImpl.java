...
public class TripEntryImpl implements TripEntry, Serializable {
  ...
  /**
   * SHADOW COPY - see BlockConfigurationEntryImpl in this package. Identical to upstream except for
   * this method. Reached from the inference path through RunTripEntry.hashCode, which folds in the
   * TripEntry it wraps.
   *
   * Note _id is not final; it is assigned by setId during a bundle build. That is safe here because
   * the graph is deserialized and then read only, but these shadows must not be on the classpath of a
   * bundle build, where an instance could be mutated after being hashed.
   */
  @Override
  public int hashCode() {
