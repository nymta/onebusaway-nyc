...
public class BlockConfigurationEntryImpl implements BlockConfigurationEntry,
  ...
  /**
   * SHADOW COPY of the class in onebusaway-transit-data-federation. Identical to upstream except for
   * this method. Present so the inference path's hash-ordered containers iterate in an order derived
   * from content rather than from Object identity, which varies between JVM runs.
   *
   * Hashes the block id rather than the block itself: BlockEntryImpl declares no hashCode either, so
   * folding it in would reintroduce the identity hash this is meant to remove. Both fields read here
   * are final. No equals() is added, so set membership is unchanged - only bucket order moves.
   */
  @Override
  public int hashCode() {
    final int prime = 31;
    int result = 1;
    result = prime * result + ((block == null) ? 0 : block.getId().hashCode());
