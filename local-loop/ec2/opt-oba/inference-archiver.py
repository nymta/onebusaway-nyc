#!/usr/bin/env python3
"""Archive the OBA post-inference ZMQ stream to hourly queueInference zips on S3.

Subscribes to the SimpleBroker fan-out of the post-inference queue (default
tcp://127.0.0.1:5567, topic "inference_queue") — the same stream the predictions
webapp consumes — and appends each message payload verbatim (it is already one
NycQueuedInferredLocationBean JSON object per message) as one line of NDJSON,
rotating on UTC hour boundaries and uploading:

  queueInference_YYYY-MM-DD_HH-00-00.zip

A passive zmq.SUB on a PUB socket: it cannot slow or drop anything upstream.
Purpose: capture real inference output as replay input for the predictions
replay engine (see onebusaway-nyc-predictions REPLAY-ENGINE-FINDINGS.md) and as
the golden format sample for the inference-side replay work. The captured file
is byte-for-byte the PIQ trace format the predictions integration tests use.

Environment:
  OBA_INF_ARCHIVER_ZMQ_HOST      default 127.0.0.1
  OBA_INF_ARCHIVER_ZMQ_PORT      default 5567
  OBA_INF_ARCHIVER_ZMQ_TOPIC     default inference_queue
  OBA_INF_ARCHIVER_DIR           default /data/inference-archive
  OBA_INF_ARCHIVER_S3_BUCKET     default mtalirr
  OBA_INF_ARCHIVER_S3_PREFIX     default oba-ec2-inference
  OBA_INF_ARCHIVER_UPLOAD        default true (false = local files only)
  OBA_INF_ARCHIVER_S3_RETRIES    default 3
  OBA_INF_ARCHIVER_LOG_STATS_SEC default 60

Restart safety matches predictions-archiver.py: on SIGTERM the partial hour is
left on disk (no zip/upload) and re-opened in append mode on the next start.
"""
...
LOG = logging.getLogger("inference-archiver")

ZMQ_HOST = os.environ.get("OBA_INF_ARCHIVER_ZMQ_HOST", "127.0.0.1")
ZMQ_PORT = os.environ.get("OBA_INF_ARCHIVER_ZMQ_PORT", "5567")
ZMQ_TOPIC = os.environ.get("OBA_INF_ARCHIVER_ZMQ_TOPIC", "inference_queue")
ARCHIVE_DIR = Path(os.environ.get("OBA_INF_ARCHIVER_DIR", "/data/inference-archive"))
S3_BUCKET = os.environ.get("OBA_INF_ARCHIVER_S3_BUCKET", "mtalirr")
S3_PREFIX = os.environ.get("OBA_INF_ARCHIVER_S3_PREFIX", "oba-ec2-inference").strip("/")
UPLOAD_ENABLED = os.environ.get("OBA_INF_ARCHIVER_UPLOAD", "true").lower() not in ("0", "false", "no")
S3_RETRIES = int(os.environ.get("OBA_INF_ARCHIVER_S3_RETRIES", "3"))
LOG_STATS_SEC = int(os.environ.get("OBA_INF_ARCHIVER_LOG_STATS_SEC", "60"))


def hour_stamp(dt: datetime) -> str:
    """UTC hour label used in queueInference object names."""
    return dt.strftime("%Y-%m-%d_%H-00-00")


def base_name(stamp: str) -> str:
    return f"queueInference_{stamp}"
...
class HourlyArchive:
    ...
    def append(self, line: str) -> Path | None:
        now = datetime.now(timezone.utc)
...
def upload_pending(archive_dir: Path) -> None:
    for zip_path in sorted(archive_dir.glob("queueInference_*.zip")):
...
def run() -> int:
    ...
    LOG.info(
        "listening %s topic=%r dir=%s bucket=s3://%s/%s upload=%s",
        endpoint,
        ZMQ_TOPIC,
        ARCHIVE_DIR,
        S3_BUCKET,
    ...
    while not stop:
        ...
        try:
            ...
            if not line:
                continue
            # archived verbatim either way; the counter just surfaces surprises
            if not line.startswith("{"):
                not_json += 1
                if not_json <= 10:
                    LOG.warning("payload does not look like JSON (%d bytes): %.80s", len(payload), line)
            finished_zip = archive.append(line)
        ...
        if now - last_stats >= LOG_STATS_SEC:
            LOG.info(
                "stats: %d messages this session, %d lines this hour, %d non-JSON payloads",
                msg_count,
                archive.line_count,
                not_json,
            )
