#!/usr/bin/env bash
#
# Build a transit-data bundle. This was previously a bare java invocation typed by hand, which made
# builds unreproducible and build-time defects easy to miss. This script records the invocation.
#
# Usage:
...
#
# SAFETY: refuses to write anywhere inside /data/oba-bundle unless ALLOW_LIVE=1. Building into a fresh
# directory is non-destructive - the running services keep using the current bundle until it is
# swapped and they are restarted.
#
set -euo pipefail

MAIN=org.onebusaway.transit_data_federation.bundle.FederatedTransitDataBundleCreatorMain
# The services are started with bundle.location=/data/oba-bundle and standalone bundle discovery, so
# that directory must contain exactly one bundle. Never build into it or alongside the live bundle;
# stage elsewhere, verify, then swap.
