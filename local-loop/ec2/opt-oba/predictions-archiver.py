...
Environment:
  ...
  OBA_ARCHIVER_STOP_ID_AGENCY default MTA (empty = archive stop ids verbatim)

Format parity: the stream's stopIds carry the bundle's per-agency prefixes (`MTA NYCT_`, `MTABC_`)
whereas prod's archive uses a single `MTA_` namespace. That is the only format difference between
the two archives, and it is normalised on write -- see normalize_stop_ids().
"""
...
# Agency prefix to force onto stopTimeUpdate.stopId so the archive matches prod's convention.
# Set empty to disable rewriting and archive the stream verbatim.
STOP_ID_AGENCY = os.environ.get("OBA_ARCHIVER_STOP_ID_AGENCY", "MTA").strip()
...
def normalize_stop_ids(feed_message: gtfs_realtime_pb2.FeedMessage) -> int:
    """Rewrite stopId agency prefixes to BusTech prod's single-namespace convention.

    We emit the bundle's per-agency ids (`MTA NYCT_401964`, `MTABC_501531`); prod's archive uses
    `MTA_<id>` throughout. Measured against a matched hour, that prefix is the only format difference
    between the two archives, and the numeric ids share one global namespace (12,909 of our 12,911
    distinct ids appear in prod's too), so re-prefixing is lossless. It also collapses the 581 stops
    we currently publish under two ids because both an NYCT and an MTA Bus route serve them.

    Only stopId is touched; tripId, routeId, vehicle.id, key order, value encodings and the entity-id
    shape already match prod exactly.

    Returns the number of stop ids rewritten.
    """
    if not STOP_ID_AGENCY:
        return 0
    rewritten = 0
    for entity in feed_message.entity:
        ...
        for stu in entity.trip_update.stop_time_update:
            ...
            if not stop_id:
                continue
            agency, sep, bare = stop_id.rpartition("_")
            if sep and agency == STOP_ID_AGENCY:
                continue
            stu.stop_id = f"{STOP_ID_AGENCY}_{bare}"
            rewritten += 1
    return rewritten
...
class HourlyArchive:
    ...
    def append(self, feed_message: gtfs_realtime_pb2.FeedMessage) -> Path | None:
        ...
        self.stop_ids_rewritten += normalize_stop_ids(feed_message)
        line = MessageToJson(
            feed_message,
...
def run() -> int:
    ...
    while not stop:
        ...
        if now - last_stats >= LOG_STATS_SEC:
            LOG.info(
                "stats: %d messages this session, %d lines this hour, %d stop ids re-prefixed to %r",
                msg_count,
                archive.line_count,
                archive.stop_ids_rewritten,
                STOP_ID_AGENCY or "(verbatim)",
            )
            last_stats = now
