...
# GTFS for stop names/locations + route names — newest GTFS*.zip = current pick (override via OBA_GTFS_ZIP)
export OBA_GTFS_ZIP="${OBA_GTFS_ZIP:-$(ls -t "$MAIN_REPO"/.context/manhattan-bundle/GTFS*.zip 2>/dev/null | head -1)}"
